...
export default function MobileOnboarding() {
  ...
  const handleLanguageSelect = (langCode: string) => {
    if (typeof window !== 'undefined') {
      sessionStorage.setItem('beteseb_onboarding_step', '3');
    }
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex items-center justify-center py-4 relative z-10 w-full overflow-hidden">
        {/* Step 1: Theme Selection (White Mode vs Dark Mode) */}
        {currentStep === 1 && (
          <div className="w-full max-w-sm flex flex-col space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-500">
            <div className="text-center space-y-2">
              ...
              <h2 className="text-xl font-bold tracking-tight">
                {locale === 'am' ? 'የሲስተም ስታይል ይምረጡ' : 'Select Theme Style'}
              </h2>
              <p className="text-xs text-white/60 leading-relaxed px-4">
                {locale === 'am' ? 'ነጭ ባክግራውንድ (White Mode) ወይም ጥቁር ባክግራውንድ (Dark Mode) ይምረጡ' : 'Choose White Mode or Dark Mode for your preferred experience.'}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-2 px-2">
              ...
              <button
                ...
                className={`p-5 rounded-3xl border-2 flex flex-col items-center justify-center gap-3 transition-all duration-300 active:scale-95 cursor-pointer ${
                  theme === 'light'
                    ? 'border-primary bg-white text-slate-900 shadow-xl shadow-primary/20 scale-105'
              ...
              >
                ...
                <div className="text-center">
                  <p className="font-bold text-sm leading-tight">{locale === 'am' ? 'ዋይት ሞድ' : 'White Mode'}</p>
                  <p className="text-[10px] opacity-70 mt-0.5">{locale === 'am' ? 'ነጭ ባክግራውንድ' : 'Light Background'}</p>
                </div>
              </button>
              ...
              <button
                ...
                className={`p-5 rounded-3xl border-2 flex flex-col items-center justify-center gap-3 transition-all duration-300 active:scale-95 cursor-pointer ${
                  theme === 'dark'
                    ? 'border-primary bg-[#1E293B] text-white shadow-xl shadow-primary/20 scale-105'
                    : 'border-white/15 bg-white/10 text-white hover:bg-white/15'
                }`}
              >
                <div className="w-12 h-12 rounded-2xl bg-indigo-950 flex items-center justify-center text-indigo-400 shadow-sm border border-indigo-800">
                  <Moon size={26} />
                </div>
                <div className="text-center">
                  <p className="font-bold text-sm leading-tight">{locale === 'am' ? 'ዳርክ ሞድ' : 'Dark Mode'}</p>
                  <p className="text-[10px] opacity-70 mt-0.5">{locale === 'am' ? 'ጥቁር ባክግራውንድ' : 'Dark Background'}</p>
                </div>
              </button>
            </div>
          </div>
        ...
        {/* Informational Slides (Steps 3 to 11) */}
        {currentStep >= 3 && currentStep <= 11 && (
          <div className="w-full max-w-sm flex flex-col items-center text-center space-y-6 px-4 animate-in fade-in zoom-in-95 duration-500">
            ...
            <div className="w-20 h-20 rounded-[2rem] bg-gradient-to-tr from-primary/10 to-primary/20 border border-primary/30 flex items-center justify-center text-primary shadow-lg shadow-primary/10 shrink-0">
              {currentStep === 3 && <Heart size={40} className="animate-pulse text-primary fill-primary/20" />}
              {currentStep === 4 && <Sparkles size={40} />}
              {currentStep === 5 && <MessageSquare size={40} />}
              {currentStep === 6 && <Users size={40} />}
              {currentStep === 7 && <BookOpen size={40} />}
              {currentStep === 8 && <HeartHandshake size={40} />}
              {currentStep === 9 && <Calendar size={40} />}
              {currentStep === 10 && <Gift size={40} />}
              {currentStep === 11 && <Coins size={40} />}
            </div>
