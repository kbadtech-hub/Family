        ...
        <div className="max-w-6xl mx-auto space-y-8 pb-24 animate-in fade-in duration-300">
          ...
          {bookingSuccess ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              {/* Counselor Info Card */}
              <div className="bg-white dark:bg-card rounded-[2.5rem] border border-border dark:border-white/10 shadow-sm p-8 space-y-6">
                ...
                <div className="text-center space-y-2">
                  <h4 className="font-black text-accent dark:text-white">{t(selectedCounselor.name_am, selectedCounselor.name)}</h4>
              ...
              {/* Booking Form */}
              <div className="lg:col-span-2 bg-white dark:bg-card rounded-[2.5rem] border border-border dark:border-white/10 shadow-sm p-8 space-y-8">
                <h3 className="text-lg font-black text-accent dark:text-white">{t('ቀጠሮ ያስይዙ', 'Book Appointment')}</h3>
  ...
  return (
    <FeatureGate featureKey="counseling" featureTitle={t('ካውንስሊንግ ሴሽን', 'Counseling Sessions')} locale={locale}>
      <div className="max-w-6xl mx-auto space-y-12 pb-24 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-5 py-3.5 rounded-2xl border border-border dark:border-white/10 bg-white dark:bg-card text-sm dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/30"
            />
          </div>
          <div className="flex gap-2 flex-wrap">
            {SPECIALIZATIONS.map(s => (
              <button
                ...
                className={`px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all ${filterSpec === s ? 'bg-primary text-white border-primary' : 'border-border dark:border-white/10 text-gray-500 hover:border-primary/40'}`}
              >
                {s === 'All' ? t('ሁሉም', 'All') : s}
              </button>
            ))}
        ...
        {/* Counselor Cards */}
        <section>
          <h3 className="text-3xl font-black text-accent dark:text-white mb-8 uppercase italic tracking-tighter">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {filtered.map(counselor => (
              <div
                ...
                className="bg-white dark:bg-card p-10 rounded-[3rem] border border-muted dark:border-white/10 shadow-sm hover:shadow-xl transition-all duration-300 group hover:-translate-y-1 cursor-pointer"
              >
