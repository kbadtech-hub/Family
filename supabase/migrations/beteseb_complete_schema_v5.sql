...
-- DATE: 2026-07-06
-- DESCRIPTION: Single-file, idempotent schema for the entire Beteseb platform.
--              Covers all tables, triggers, RLS policies, indexes, grants,
--              seed data, telemetry, AI chatbot KB, and support system.
...
-- AI Chatbot self-learning KB: populated automatically when admin resolves tickets
...
-- Privacy-isolated telemetry: only the two chatting participants can ever read this
...
BEGIN
  ...
  SET coin_balance = public.user_wallets.coin_balance + NEW.amount, updated_at = NOW();
  RETURN NEW;
END;
 LANGUAGE plpgsql SECURITY DEFINER;
...
RETURNS TRIGGER AS 
BEGIN
  INSERT INTO public.user_wallets (id, coin_balance) VALUES (NEW.id, 0) ON CONFLICT (id) DO NOTHING;
  RETURN NEW;
END;
 LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  ...
  VALUES (NEW.id, NEW.email, NEW.phone, COALESCE(NEW.raw_user_meta_data->>'full_name', ''), 1)
...
CREATE OR REPLACE TRIGGER on_friendship_accepted_telemetry
...
CREATE POLICY "Participants can access Wali Rooms" ON public.wali_rooms FOR ALL USING (auth.uid() = candidate1_id OR auth.uid() = candidate2_id OR EXISTS (SELECT 1 FROM public.guardians WHERE id = guardian1_id AND user_id = auth.uid()) OR EXISTS (SELECT 1 FROM public.guardians WHERE id = guardian2_id AND user_id = auth.uid()));
CREATE POLICY "Participants can manage messages in their Wali Room" ON public.wali_messages FOR ALL USING (EXISTS (SELECT 1 FROM public.wali_rooms r WHERE r.id = room_id AND (auth.uid() = r.candidate1_id OR auth.uid() = r.candidate2_id OR EXISTS (SELECT 1 FROM public.guardians WHERE id = r.guardian1_id AND user_id = auth.uid()) OR EXISTS (SELECT 1 FROM public.guardians WHERE id = r.guardian2_id AND user_id = auth.uid()))));
CREATE POLICY "Admin full access for SMS queue" ON public.sms_queue FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin')));
...
CREATE POLICY "Admin full access to tickets" ON public.support_tickets FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin')));
CREATE POLICY "Users can view replies to own tickets" ON public.support_replies FOR SELECT USING (EXISTS (SELECT 1 FROM public.support_tickets t WHERE t.id = ticket_id AND t.user_id = auth.uid()));
CREATE POLICY "Admin full access to replies" ON public.support_replies FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin')));
CREATE POLICY "Public read resolved_kb" ON public.resolved_kb FOR SELECT USING (true);
CREATE POLICY "Admin insert resolved_kb" ON public.resolved_kb FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin')));
CREATE POLICY "telemetry_isolation_policy" ON public.interaction_telemetry FOR ALL TO authenticated USING (EXISTS (SELECT 1 FROM public.friendships f WHERE f.id = friendship_id AND (f.sender_id = auth.uid() OR f.receiver_id = auth.uid())));
...
INSERT INTO public.gift_catalog (id, name_en, name_am, name_om, name_ti, name_so, name_ar, image_url, coin_price, category, unlock_level, commission_rate, base_delivery_fee) VALUES
  ...
  ('66666666-6666-6666-6666-666666666666', 'Luxury Poodle Puppy',    'የቅንጦት ቡችላ (ፖፒ)',   'Saree Luxury',               'ቅንጡ ዕትብት',   'Ey puppy',               'جرو بودل فاخر',          'luxury_puppy',    500, 'pets',     4, 0.25,150.00),
