...
CREATE POLICY "Users can upload own verifications" ON storage.objects 
FOR INSERT WITH CHECK (
    bucket_id IN ('verifications', 'id-verification') AND auth.role() = 'authenticated'
);

CREATE POLICY "Users can view own verifications" ON storage.objects 
FOR SELECT USING (
...
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS gallery_photos TEXT[] DEFAULT '{}';
